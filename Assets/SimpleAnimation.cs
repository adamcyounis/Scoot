...
public class SimpleAnimation {
    ...
    public enum TimeKind { Scaled, Fixed, Unscaled, FixedUnscaled };
    ...
    public void Animate(bool overRide) {
        Animate(startValue, targetValue, overRide);
    }

    public void Animate(float startValue_, float targetValue_, bool overRide) {
        Animate(startValue_, targetValue_, duration, overRide);
    }

    public void Animate(float startValue_, float targetValue_, float duration_, bool overRide) {
        Animate(startValue_, targetValue_, duration_, overRide, defaultFunction);
    }

    public void Animate(float startValue_, float targetValue_, float duration_, bool overRide, Curve newFunction) {
    ...
    public void Animate(float startValue_, float targetValue_, float duration_, bool overRide, AnimationCurve curve_) {
        if (overRide || ((!animating) && (targetValue_ != value))) {
            value = startValue_;
            startValue = startValue_;
            targetValue = targetValue_;
    ...
    public void Update(float elapsedTime_) {    //elapsedTime = normalised timescale 
        value = Evaluate(elapsedTime_);
        if (elapsedTime_ >= 1) {
            if (animating) {
    ...
    public void SetTimeScale(bool fixedT, bool unscaledT) {
        if (fixedTime) {
            if (unscaledTime) {
                kind = TimeKind.FixedUnscaled;
            } else {
                kind = TimeKind.Fixed;
            }
        } else {
            if (unscaledTime) {
                kind = TimeKind.Unscaled;
            } else {
                kind = TimeKind.Scaled;
            }
        }
    }
    public float GetTime() {
        switch (kind) {
            ...
            case TimeKind.Unscaled:
                return Time.unscaledTime;

            case TimeKind.FixedUnscaled:
                return Time.fixedUnscaledTime;

            case TimeKind.Scaled:
                return Time.time;
